...
export class NekoAssist implements QBotPlugin {
  ...
  onGroupMessage = async (data: GroupMessageEvent, info: QBotMessageInfo) => {
    ...
    if (!data.raw_message.includes(`[CQ:at,qq=${this.qbot.account}]`)) return;
    ...
    const messageList: AIMessageItem[] = [
      ...
      { role: 'user', content: info.resolvedRawMessage },
    ];
    ...
    if (!success) {
      await naplink.sendGroupMessage(data.group_id, `接口请求失败了喵\n${res}`);
      return;
    }
    ...
    try {
      ...
      if (json.action === 'reply') {
        ...
        await naplink.sendGroupMessage(data.group_id, reply);
        qbot.history.push(`${currentTime} ${qbot.account}: ${reply}`);
        qbot.saveHistory();
      }
    } catch (e) {
      await naplink.sendGroupMessage(data.group_id, `解析回复为 JSON 数据失败了喵\n${e}`);
    }
  };
}
